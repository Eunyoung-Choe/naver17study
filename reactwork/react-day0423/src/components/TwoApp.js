import { DeleteForeverOutlined, DeleteForeverRounded, PhoneAndroidSharp } from '@mui/icons-material';
...
const TwoApp = () => {
    ...
    //메세지 입력후 엔터 누르면 배열에 추가
    const addMessageEvent=(e)=>{
        if(e.key==='Enter'){
            //alert(e.target.value+","+e.key);

            //리액트에서는 배열에 추가시 push 가 아닌 concat 으로 추가한다
    ...
    const deleteMessage=(i)=>{
        ...
        //2. filter  를 이용한 방법(i번지만 삭제해서 새로운 배열로 리턴)
        ...
        <div style={{width:'400px'}}>
            ...
            <h6 style={{backgroundColor:'orange'}}> msg 배열 데이터 (총{msg.length}개)</h6>
            {
                msg.map((m,i)=>
                    <h5 key={i}>
                        {i+1} : {m}
                        {/* <DeleteForeverRounded style={{float:'right',cursor:'pointer',color:'red'}}
                        onClick={()=>deleteMessage(i)}/> */}
                        <i class="bi bi-trash-fill" style={{float:'right',cursor:'pointer',color:'red'}}
