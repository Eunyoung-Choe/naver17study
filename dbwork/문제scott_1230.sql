...
-- 14. MGR을 GROUP로 인원수와 평균 SAL을 구하시오(평균 SAL은 무조건 올림으로)
    select count(*), ceil(avg(sal)), mgr from emp group by mgr; --평균 소수점 쓰려면 round()로.
...
-- 18. JOB 직업별로 인원수와 최고 연봉을 출력하는데 직업의 오름차순으로 출력
    select job 직업, count(*), max(sal) from emp group by job order by job;

-- 19. || 연산자를 이용하여 다음과 같이 하나의 컬럼으로 출력하시오
----      (예) SCOTT의 직업은 CLERK이며 입사년도는 1989년 05월입니다.
    select ename||'의 직업은 '||job||'이며 입사년도는 '||to_char(hiredate, 'yyyy-mm-dd')||'입니다.' 자기소개 from emp;