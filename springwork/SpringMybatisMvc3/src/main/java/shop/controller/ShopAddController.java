...
public class ShopAddController {
   ...
   public String insert(
      ...
      @RequestParam("upload") List<MultipartFile> uploadList //addform에 Multiple로 적용했기 때문에 여기도 List로 받아야함
      ) {
      //업로드할 save 경로 구하기
      ...
      for(MultipartFile upload:uploadList) {
         ...
         // String uploadFilename=UUID.randomUUID() + "."+(upload.getOriginalFilename().split("\\.") <- 배열로 만들어짐
      ...
      return "redirect:./list"; //띄어쓰기 있으면 경로로 인식해서 넣으면 X
   }
