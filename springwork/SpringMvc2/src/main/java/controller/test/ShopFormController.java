...
public class ShopFormController {
   ...
   public String list1(Model model,
         ...
         //방법2 : 폼태그의 name 읽을 변수명이 같을 경우 name은 생략 가능
//         @RequestParam String myid,
//         @RequestParam String mypass       
         
         //방법3 : 이름이 같을경우 @RequestParam도 생략가능 (권장X-에러날때도있음)
         String myid,
         String mypass,
   ...
   //public String list2(@ModelAttribute ShopDto dto) { //dto로 통쨰로 읽어서 모델에 저장(저장시 (첫글자 소문자)shopDto로 저장)
   public String list2(@ModelAttribute("dto") ShopDto dto) {//dto로 통쨰로 읽어서 모델에 저장(저장시 dto로 저장)   
      return "shop/list2";
   }
   
   @PostMapping("/process3")
   public String list3(Model model,
		   @RequestParam Map<String, String> map) // 폼 태그의 name이 key값으로, 입력값이 value로 넘어온다.
   {
